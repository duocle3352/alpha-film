...
function SlideComp({ listItem, count }) {
    ...
    return (
        <ul className={cx('list-item')} style={{ transform: `translateX(calc(-${sliderWidth}px * ${count}))` }}>
            {listItem.map((item) => (
                <li key={item.id} className={cx('item')} ref={sliderRef}>
                    <ImageOriginal
                        className={cx('img')}
                    ...
                    <div className={cx('info')}>
                        ...
                        <div className={cx('info__inner')}>
                            ...
                            <h4 className={cx('subtitle')}>{`${item.vote_average.toFixed(1)}/10`}</h4>
                        </div>
                        ...
                        <div className={cx('btn')}>
                            ...
                            <Button to={`${item.media_type}/${item.id}`} outline large leftIcon={<BsPlayCircle />}>
                                Play Trailer
                            </Button>
                        </div>
                    </div>
